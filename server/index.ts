...
// Lightweight health endpoints for ops/infra
// Note: Fly.io uses /resources/healthcheck already; these are additional
// endpoints to satisfy /healthz and /readyz expectations in docs/PRD.
app.get('/healthz', (_req, res) => {
	res.setHeader('X-Healthcheck', 'true')
...
app.get('/readyz', async (_req, res) => {
	res.setHeader('X-Healthcheck', 'true')
	try {
		// Basic readiness signal: ensure we can resolve the server build
		const { error } = await getBuild()
